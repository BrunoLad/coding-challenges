...
type ExtractToObject<T, U extends keyof T> = Required<{
  ...
  // Satisfy cases where inner property only has 1 key
  type ExtractToObject1<T, U extends keyof T> = {
    [key in keyof T as key extends U ? keyof T[U] : key]: key extends U ? T[U][keyof T[U]] : T[key]
  }
  
  type ExtractToObject3<T, U extends keyof T> = Required<Omit<T & T[U], U>>;
  
  const test: ExtractToObject<test1, 'myProp'> = {
    foo: '2',
    id: '1'
  };
