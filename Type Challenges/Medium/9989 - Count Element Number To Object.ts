...
type MapCount<T extends unknown[]> = {
  [key in keyof T as T[key] extends number | string ? Equal<key, 'length'> extends false ? T[key] : never : never]: CountElements<T, T[Exclude<key, number>]>
}
type Flatten<T extends unknown[]> = T extends [infer Head, ...infer Tail]
  ? Head extends unknown[]
    ...
    : [Head, ...Flatten<Tail>]
  : [];
type CountElements<T extends unknown[], E, A extends unknown[] = []> = T extends [infer Head, ...infer Tail]
  ? Head extends E
    ? CountElements<Tail, E, [Head, ...A]>
    : CountElements<Tail, E, A>
  : A['length'];
type CountElementNumberToObject<T extends unknown[]> = T extends never[] ? {} : MapCount<Flatten<T>>;
