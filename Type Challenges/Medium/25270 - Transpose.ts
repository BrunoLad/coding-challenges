...
type ExtractFirst<M> = M extends [infer Head extends unknown[], ...infer Tail extends unknown[]]
  ? Head[number] extends never
    ? []
    : [Head[0], ...ExtractFirst<Tail>]
  : [];
type RemoveFirst<M> = M extends[infer Head extends unknown[], ...infer Tail extends unknown[]]
  ? Head extends [infer First, ...infer Rest]
    ? [Rest, ...RemoveFirst<Tail>]
    : []
  : [];
type Transpose<M extends number[][]> = M extends [infer Head extends number[], ...infer Tail]
  ? Head[number] extends never
    ? []
    : [ExtractFirst<M>, ...Transpose<RemoveFirst<M>>]
  : [];
