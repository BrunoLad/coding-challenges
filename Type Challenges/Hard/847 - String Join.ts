...
type Join<T, SEP extends string> = T extends [infer Head extends string, ...infer Tail]
  ? `${Head}${SEP}${Join<Tail, SEP>}`
  : '';
type ReverseString<S extends string> = S extends `${infer Head}${infer Tail}`
  ? `${ReverseString<Tail>}${Head}`
  : '';
type RemoveFirstChar<S extends string> = S extends `${infer Head}${infer Tail}`
  ? Tail
  : S;
// {[key in typeof parts[number]]: key }
declare function join<T extends string>(delimiter: T): <X>(...parts: X extends readonly string[] ? X : []) => T extends ''
  ? Join<X, T>
  : ReverseString<RemoveFirstChar<ReverseString<Join<X, T>>>>;
