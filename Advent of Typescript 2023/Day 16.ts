type OnlySantaAllowed<T, K extends keyof T> = "🎅🏼" extends T[K] ? K : never;
type ToNumber<N> = N extends `${infer V extends number}` ? V : never;
type FindSanta<T extends string[][], A extends unknown[][] = []> = T extends [
	infer Head extends unknown[],
	...infer Tail extends string[][],
]
	? ToNumber<
			keyof {
				[K in keyof Head as OnlySantaAllowed<Head, K>]: K;
			}
		> extends never
		? FindSanta<Tail, [Head, ...A]>
		: [
				A["length"],
				ToNumber<
					keyof {
